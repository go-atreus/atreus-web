...
export async function query(body: QueryParam<SysDictQo>) {
  return request<R<PageResult<SysDictVo>>>('/api/system/dict/list', {
    method: 'POST',
    data: body,
  });
}
...
export async function validHash(body: SysDictDataHash) {
  return request<R<string[]>>('/api/system/dict/invalid-hash', {
    method: 'POST',
    data: body,
  });
}
...
export async function dictData(body: string[]) {
  return request<R<SysDictData[]>>(`/api/system/dict/data?dictCodes=${body.join(',')}`, {
    method: 'GET',
  });
}
